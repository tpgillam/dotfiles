...
# Check if the ssh-agent is running with the PID from the environment file
if _is_system_rebooted || ! ps -p "$SSH_AGENT_PID" --no-headers -o comm= | grep -q "^ssh-agent$"; then
    _start_agent
fi
